...
**Response**: I notice the movies that passed the Bechdel test have both smaller median budget and median gross.
...
**Response**: I notice that the maximum median budget and median gross are `notalk`. It seems there is a non-linear correlation between the granular indicator and both the median budget and median gross. A correlation leaning towards `binary = FAIL`.
...
**Response**: I notice a positive correlation between Return on Investment and the `clean_test`, granular indicator of `binary = PASS`.
